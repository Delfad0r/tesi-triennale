...
\end{proposition}
\begin{proof}
Sia $x\in X$ un punto. Poiché $E_{x,X}$ è contrattile, tutti i suoi moduli di omologia sono finitamente generati (in particolare $H_0(E_{x,X};A)=0$ e $H_i(E_{x,X};A)=A$ per $i>0$). Applicando la \thref{finitely-generated-homology-groups} alla fibrazione $\fibration{\Omega}{E_{x,X}}{X}$ si ottiene immediatamente la tesi.
\end{proof}
