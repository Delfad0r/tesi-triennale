...
\begin{proposition}
Sia $X$ uno spazio topologico connesso per archi e semplicemente connesso, $x\in X$; sia $\Omega=\Omega X$. Allora esiste una successione spettrale $E^{p,q}_r$ tale che $E^{p,q}_2=H_p(X,H_q(\Omega))$, il cui gruppo terminale $E_\infty$ è isomorfo al gruppo graduato associato a una filtrazione di $H(E_{x,X})$.
\end{proposition}
\begin{proof}
Consideriamo la fibrazione $E_{x,X}\to X$ della \thref{path-space-fibration} (dove abbiamo identificato $\{x\}\times X$ con $X$). Le fibre sono spazi del tipo $E_{x,y}$, dunque omotopicamente equivalenti a $\Omega$. Poiché $X$ è semplicemente connesso, $\Omega$ è connesso per archi. Siamo dunque nelle condizioni di applicare \missing{}, da cui segue immediatamente la tesi.
\end{proof}
