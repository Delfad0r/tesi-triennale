...
\section{Definizioni}
Per tutta la trattazione, $R$ denoterà un dominio a ideali principali. Molti risultati sarebbero validi anche per anelli $R$ generici, ma non ci preoccuperemo di questa differenza.
\begin{definition}
Si dice successione spettrale (omologica) una famiglia di $R$-moduli $E^{p,q}_r$ indicizzata da $p,q\in\ZZ,r\in\NN$ dotata di omomorfismi $\map{d^{p,q}_r}{E^{p,q}_r}{E^{p-r,q+r-1}_r}$ (detti mappe di bordo) che soddisfa le seguenti proprietà:
\begin{enumerate}
...
\section{Successione spettrale associata a un complesso filtrato}
\label{sec:spectral-sequence-filtered-complex}
\begin{definition}
Sia $C_\bullet$ un complesso di $R$-moduli con $C_n=0$ per $n<0$. Si dice filtrazione (crescente) di $C_\bullet$ una successione di sottocomplessi $C^p_\bullet\subs C_\bullet$ indicizzati da $p\in\ZZ$ che soddisfa le seguenti proprietà:
\begin{enumerate}
...
\end{diagram}
Pertanto possiamo definire i moduli
\begin{align*}
...
I moduli $E^{p,q}_r$, insieme alle mappe di bordo $d^{p,q}_r$, formano una successione spettrale con modulo terminale $E^{p,q}_\infty$. Inoltre $E^{p,q}_r\converges H_{p+q}(C_\bullet)$.
\end{proposition}
\begin{proof}
...
\end{align*}
\end{enumerate}
Dunque i moduli $E^{p,q}_r$ formano una successione spettrale. Verifichiamo ora che $E^{p,q}_\infty$ sono effettivamente i moduli terminali di tale successione. Osserviamo che per $p>r$ vale $Z^{p,q}_r=Z^{p,q}_\infty$, mentre per $r\ge q+1$ vale $B^{p,q}_r=B^{p,q}_\infty$. Dunque per $r$ sufficientemente grande vale
$$
...
\begin{definition}
Si dice successione spettrale coomologica una famiglia di $R$-moduli $E^{p,q}_r$ indicizzata da $p,q\in\ZZ,r\in\NN$ dotata di omomorfismi $\map{d^{p,q}_r}{E^{p,q}_r}{E^{p+r,q-r+1}_r}$ (detti mappe di bordo) che soddisfa le seguenti proprietà:
\begin{enumerate}
\item $E^{p,q}_r=0$ per $p<0$ o $q<0$;
\item $d^{p+r,q-r+1}_r\circ d^{p,q}_r=0$;
\item $E^{p,q}_{r+1}=\ker d^{p,q}_r/\im d^{p-r,q+r-1}_r$.
\end{enumerate}
\end{definition}
Come nel caso omologico, per $p,q$ fissati esiste un $R$-modulo $E^{p,q}_\infty$ (detto modulo terminale) tale che per $r$ sufficientemente grande vale $E^{p,q}_r=E^{p,q}_\infty$.
\begin{definition}
Si dice che una successione spettrale coomologica $E^{p,q}_r$ converge a una famiglia di $R$-moduli $H^n$ (e si scrive $E^{p,q}_r\converges H^{p+q}$) se esistono filtrazioni
$$
...
\begin{definition}
Sia $C^\bullet$ un cocomplesso di $R$-moduli con $C^n=0$ per $n<0$. Si dice filtrazione (decrescente) di $C^\bullet$ una successione di sottocomplessi $C_p^\bullet\subs C^\bullet$ indicizzati da $p\in\ZZ$ che soddisfa le seguenti proprietà:
\begin{enumerate}
...
I moduli $E^{p,q}_r$, insieme alle mappe di bordo $d^{p,q}_r$, formano una successione spettrale coomologica con modulo terminale $E^{p,q}_\infty$. Inoltre $E^{p,q}_r\converges H^{p+q}(C^\bullet)$.
\end{proposition}

\subsection{Struttura moltiplicativa}
Se $C^\bullet$ è munito di una struttura di anello, come spesso accade nel contesto topologico, allora ne esiste anche una indotta sui termini della successione spettrale associata. Più precisamente, supponiamo che sia definito un prodotto $R$-bilineare $\map{\cupp}{C^n\times C^m}{C^{n+m}}$ che soddisfa le seguenti proprietà:
\begin{enumerate}
...
\end{enumerate}
Si verifica \todo{Farlo a un certo punto.} allora che $\cupp$ passa al quoziente, inducendo un prodotto $E^{p,q}_r\times E^{p',q'}_r\to E^{p+p',q+q'}_r$; inoltre le mappe di bordo $d^{p,q}_r$ risultano essere antiderivazioni rispetto al grado totale. È dunque conveniente definire $E_r=\Dirsum_{p,q}E^{p,q}_r$, che sotto queste ipotesi diventa una $R$-algebra bigraduata (da $p$ e da $q$).

\subsection{Cocomplesso filtrato duale}
Esibiamo ora una costruzione che permette di associare una successione spettrale coomologica a un complesso filtrato $(C_\bullet, d)$. Sia $G$ un $R$-modulo. Consideriamo il cocomplesso $C^\bullet=\Hom(C_\bullet,G)$ con mappe di bordo $-\circ d$. Il cocomplesso $C^\bullet$ eredita una filtrazione decrescente $C^n_p=\Ann C_n^{p-1}$ (le proprietà richieste si verificano facilmente), a partire dalla quale possiamo costruire una successione spettrale coomologica $E^{p,q}_r\converges H^{p+q}(C_\bullet;G)$.

\section{Successione spettrale associata a un rivestimento universale}
\todo{Breve introduzione.}
\begin{proposition}\thlabel{spectral-sequence-of-universal-covering}
Siano $G$ un $R$-modulo, $X$ uno spazio topologico che ammette un rivestimento universale $\map{p}{T}{X}$. Sia $\pi$ il gruppo fondamentale di $X$. Allora esiste una successione spettrale $E^{p,q}_r\converges H_{p+q}(X;G)$ tale che $E^{p,q}_2=H_p(\pi;H_q(T;G))$.
\end{proposition}
Precisiamo che nell'enunciato abbiamo denotato con $H_p(\pi;H_q(T;G)))$ il $p$-esimo modulo di omologia di $\pi$ a coefficienti nel $\pi$-modulo $H_q(T;G)$.
\begin{corollary}\thlabel{}
Supponiamo che $G$ sia un campo; supponiamo inoltre che il gruppo fondamentale $\pi$ sia il gruppo additivo $\ZZ$ e che agisca banalmente sui moduli di omologia di $T$. Allora
$$
...
\end{corollary}
\begin{proof}
Consideriamo la successione spettrale $E^{p,q}_r$ data dalla \thref{spectral-sequence-of-universal-covering}. È ben noto che, se l'azione di $\ZZ$ su $H_q(T;G)$ è banale, allora
$$
...
\end{proof}
\begin{corollary}
Supponiamo che $\pi$ sia finito e che agisca banalmente sui gruppi di omologia di $T$; supponiamo inoltre che $G$ sia un campo la cui caratteristica non divide l'ordine di $\pi$. Allora $H_i(X;G)=H_i(T;G)$ per ogni $i$.
\end{corollary}
\begin{proof}
Consideriamo la successione spettrale $E^{p,q}_r$ data dalla \thref{spectral-sequence-of-universal-covering}. È ben noto che, con le ipotesi fatte, vale
$$
H_p(\pi;H_q(T;G))=
\begin{cases}
H_q(T;G)&p=0\\
