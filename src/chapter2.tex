...
Denotiamo con $Q_n(X)$ l'$R$-modulo libero avente per base l'insieme dei cubi singolari di dimensione $n$, con $D_n(X)$ l'$R$-modulo libero avente per base l'insieme dei cubi degeneri di dimensione $n$. Per definire il complesso $Q_\bullet(X)$ è necessario costruire mappe di bordo $\map{d_n}{Q_n(X)}{Q_{n-1}(X)}$.
...
$$
estendendola per $R$-linearità a tutto $Q_n(X)$. È immediato verificare che $\lambda^\epsilon_i\lambda^{\epsilon'}_j=\lambda^{\epsilon'}_{j-1}\lambda^\epsilon_i$; un semplice conto mostra allora che $d_nd_{n+1}=0$. Abbiamo così definito il complesso $Q_\bullet(X)$. Si vede inoltre che $D_\bullet(X)$ è un sottocomplesso di $Q_\bullet(X)$: se $u$ è un cubo degenere di dimensione $n$, allora anche $\lambda^\epsilon_iu$ è degenere per $0\le i<n$, mentre $\lambda^0_nu=\lambda^1_nu$, pertanto $du$ è degenere.
\begin{definition}
Si dice complesso singolare (cubico) di $X$ il complesso $C_\bullet(X)=Q_\bullet(X)/D_\bullet(X)$. I suoi moduli di omologia e coomologia a coefficienti in un $R$-modulo $G$ si dicono moduli di omologia e coomologia singolare (cubica) di $X$ a coefficienti in $G$.
\end{definition}
...
\end{align*}
Osserviamo che $C^n(X;G)$ può essere interpretato come il modulo delle funzioni dai cubi di dimensione $n$ in $G$ che sono nulle sui cubi degeneri.

Siano inoltre $H_*(X;G)=\Dirsum_{n\ge 0}H_n(X;G)$, $H^*(X;G)=\Dirsum_{n\ge 0}H^n(X;G)$. Ometteremo il modulo dei coefficienti $G$ quando $G=R$, scrivendo semplicemente, ad esempio, $H_*(X)$ per indicare l'omologia di $X$ a coefficienti in $R$. Esattamente come nel caso della teoria singolare classica, $H^*(X)$ acquisisce una struttura di $R$-algebra graduata. Si definisce il \cupproduct{} come segue: se $u$ è un cubo di dimensione $p+q$ e $f,g$ sono cocatene di dimensione $p,q$ rispettivamente, allora
$$
...
Denotiamo con $H_*^\Delta(X;G),H^*_\Delta(X;G)$ l'omologia e la coomologia singolare standard. Allora $H_*(X;G)\iso H_*^\Delta(X;G)$ come moduli graduati, e $H^*(X;G)\iso H_\Delta^*(X;G)$ come algebre graduate.
\end{proposition}
...
Supponiamo che $X$ sia connesso per archi; sia $x\in X$ un punto fissato. Allora i moduli di omologia e coomologia (cubica)  di $X$ rimangono inalterati se ci si limita a considerare cubi singolari aventi tutti i vertici in $x$.
\end{proposition}
...
Ci proponiamo ora di mostrare come il gruppo fondamentale di $B$ agisca sui moduli di omologia e coomologia di $F$.
...
Da questa proposizione segue che le mappe indotte in omologia $\map{\varphi_*}{H_q(E^{p,\bullet}_0)}{H_q(J^p_\bullet)}$ sono isomorfismi. Osserviamo però che $H_q(E^{p,\bullet}_0)=E^{p,q}_1$, mentre dal fatto che $C_p(B)$ è un modulo libero segue
$$
...
Supponiamo che il gruppo fondamentale di $B$ agisca banalmente sui moduli di omologia di $F$. Allora il seguente diagramma commuta
\begin{diagram}
...
Supponiamo che il gruppo fondamentale di $B$ agisca banalmente sui moduli di omologia di $F$. Allora $\varphi_*$ induce un isomorfismo $E^{p,q}_2\iso H_p(B;H_q(F))$.
\end{proposition}

Osserviamo che i risultati ottenuti fino a questo punto rimangono validi se consideriamo l'omologia a coefficienti in un $R$-modulo $G$: a ogni spazio fibrato si può associare una successione spettrale $E^{p,q}_r$ (derivante dall'omologia a coefficienti in $G$) i cui termini con $r=2$ sono isomorfi a $H_p(B;H_q(F;G))$.

\section{Successione spettrale coomologica di uno spazio fibrato}
Come è lecito aspettarsi, dato uno spazio fibrato è possibile associarvi anche una successione spettrale coomologica. Sia $G$ un $R$-modulo. Partendo dal complesso filtrato $C_\bullet$ procediamo come mostrato in \missing{}, costruendo il cocomplesso $C^\bullet=\Hom(C_\bullet, G)$ e filtrandolo mediante i sottocomplessi $C^\bullet_p=\Ann C^{p-1}_\bullet$. Otteniamo così una successione spettrale coomologica $E^{\ast p,q}_r \converges H^{p+q}(E)$.
...
Supponiamo che il gruppo fondamentale di $B$ agisca banalmente sui moduli di coomologia di $F$. Allora $\psi^*$ induce un isomorfismo $E^{\ast p,q}_2\iso H^p(B;H^q(F;G))$.
\end{proposition}
Supponiamo ora che $G=R$. Allora i moduli di coomologia di $E$ acquistano una struttura di $R$-algebra, e lo stesso accade per i moduli $E^\ast_r$. Infatti il \cupproduct{} definisce una struttura di anello su $C^\bullet(E)$; si verifica facilmente che tale prodotto soddisfa la condizione $C^n_p\cupp C^m_q\subs C^{m+n}_{p+q}$, dunque come mostrato in \missing{} i termini della successione spettrale $E^\ast_r$ ereditano una struttura di anello per la quale le mappe di bordo sono antiderivazioni. È naturale domandarsi come questa struttura si trasformi mediante l'isomorfismo della \thref{cohomological-spectral-sequence-of-fibration-E-2}.
Definiamo
$$
H^*(B;H^*(F))=\Dirsum_{p,q}H^p(B;H^q(F)).
$$
La proposizione precedente ci dice che $\psi^*$ induce un isomorfismo $E^\ast_2\iso H^*(B;H^*(F))$ di $R$-moduli.
...
$$
Vedremo con la proposizione seguente che $\psi^*$ è anche un isomorfismo di $R$-algebre, a meno di un segno.

\begin{proposition}\thlabel{cohomological-spectral-sequence-of-fibration-ring}
Supponiamo che il gruppo fondamentale di $B$ agisca banalmente sui moduli di coomologia di $F$. Siano $g\in E^{\ast p,q}_2,g'\in E^{\ast p',q'}_2$. Allora $\psi^*(g\cdot g')=(-1)^{p'q}\psi^*(g)\cdot\psi^*(g')$.
\end{proposition}
\begin{proof}
...
Supponiamo che il gruppo fondamentale di $B$ agisca banalmente sui moduli di coomologia di $F$. Allora gli anelli $E^\ast_r$ con $r\ge 2$ sono anticommutativi rispetto al grado totale $n=p+q$.
\end{corollary}
\begin{proof}
...
&=(-1)^{nn'}g'\cdot g,
\end{align*}
dove abbiamo sfruttato la \thref{cohomological-spectral-sequence-of-fibration-ring} e la proposizione analoga per $\varphi^*$. Dunque la tesi è vera per $r=2$. Ma allora è vera per ogni $r\ge 2$, poiché la struttura di anello di $E^\ast_r$ si ottiene per passaggio al quoziente da quella di $E^\ast_{r-1}$.
\end{proof}

Concludiamo la sezione con un risultato che permette di semplificare, sotto alcune ipotesi, il calcolo del termine $E^\ast_2$ della successione spettrale. Se $M$ e $N$ sono due $G$-algebre graduate, possiamo definire su $M\tensor N$ una struttura di $G$-algebra come segue: se $x,x'\in M$ e $y,y'\in N$ sono elementi omogenei di gradi, rispettivamente $p,p',q,q'$, allora
$$
(x\tensor y)(x'\tensor y')=(-1)^{p'q}(xx'\tensor yy').
$$
\begin{proposition}\thlabel{cohomological-spectral-sequence-of-fibration-tensor-product}
Sia $G$ un PID. Supponiamo che sia soddisfatta una delle due condizioni seguenti:
\begin{enumerate}
...
$$
È evidente che $\varphi^*\circ i^*$ preserva la struttura moltiplicativa, dunque è sufficiente mostrare che, sotto le ipotesi dell'enunciato, $i^*$ è un isomorfismo di $G$-moduli.
\begin{enumerate}
